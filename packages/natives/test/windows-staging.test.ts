...
describe("pi-natives version sentinel", () => {
	it("Rust `js_name` matches the native compatibility version independently of the application version", async () => {
		...
		const expected = `__piNativesV${packageJson.nativeCompatibilityVersion.replace(/[^A-Za-z0-9]/g, "_")}`;
