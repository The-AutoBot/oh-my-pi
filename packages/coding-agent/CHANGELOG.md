...
- Collaboration pending-input events now identify select versus editor prompts without exposing prompt content.

### Fixed

- Collaboration reuse now waits only for a ready session, preserves reconnect and stop lifecycle identity, and rejects stale session or start requests.
