...
export class CollabController {
	...
	async start(options: CollabStartOptions): Promise<CollabHost> {
		...
		const started = this.#ops.then(async () => {
			...
			return this.#launch(options.access, stopEpoch, options.relay, options.webUrl);
		});
	...
	/**
	 * Ensure the process hosts a control room for extensions. Concurrent calls
	 * share the first launch and report every later caller as a reuse.
	 */
	async ensure(options: CollabStartOptions): Promise<CollabEnsureResult> {
		...
		const started: Promise<CollabEnsureResult> = this.start({ ...options, access: "control" }).then(host => ({
			host,
			reused: false,
		}));
	 ...
	 */
	async #launch(access: CollabAccess, stopEpoch: number, relay?: string, webUrl?: string): Promise<CollabHost> {
		if (this.#shutdown) throw new CollabHostStoppedError("collab controller shut down");
		if (stopEpoch !== this.#stopEpoch) throw new CollabHostStoppedError("collab controller stopped");
	...
	#handleHostConnectionState(host: CollabHost, state: CollabConnectionState): void {
		...
		this.#lifecycleTail = queued.catch(error => {
			logger.warn("Collab lifecycle handler failed", { error: String(error) });
		});
		return queued;
	}
