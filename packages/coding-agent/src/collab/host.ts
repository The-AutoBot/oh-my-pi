...
import type { CollabInputStateEvent } from "../extensibility/extensions/types";
...
export interface CollabHostOptions {
	...
	onConnectionState?: (host: CollabHost, state: CollabConnectionState) => void | Promise<void>;
}
...
export class CollabUnavailableError extends Error {
	readonly code: `collab-${CollabUnavailableCode}`;

	constructor(reason: CollabUnavailableCode) {
		super(`Collaboration unavailable (${reason}).`);
		this.name = "CollabUnavailableError";
		this.code = `collab-${reason}`;
	}
}
...
export class CollabHost {
	...
	readonly #onStopped?: (host: CollabHost, reason: CollabStopReason) => void | Promise<void>;
	readonly #onConnectionState?: (host: CollabHost, state: CollabConnectionState) => void | Promise<void>;
	...
	/** Session this room mirrors; fixed at construction. */
	...
	/** Unique to this host process generation; never carries room capability material. */
	get hostId(): string {
		return this.#hostId;
	}
	...
	get isReady(): boolean {
		return !this.ending && this.#sessionStillCurrent() && this.#relayConnected && this.#socket?.isOpen === true;
	}

	/**
	 * Wait for a live relay socket for this host's original session. Reuse callers
	 * must use this rather than treating an allocated host object as availability.
	 */
	async waitForReady(timeoutMs = CONNECT_TIMEOUT_MS): Promise<void> {
		...
		if (this.ending || (!this.#socket && !this.#abortStart)) throw collabUnavailableError("stopped");
	...
	#syncInputState(): void {
		...
		const inputType = needsInput ? this.#pendingUi.values().next().value!.request.kind : null;
		if (needsInput === this.#needsInput && inputType === this.#lastInputType) return;
	...
	#sendUiRequestToPeer(request: CollabUiRequest, peerId: number): void {
		...
		if (!peer?.canWrite || peer.deliveredUiRequests.has(request.reqId)) return;
		peer.deliveredUiRequests.add(request.reqId);
		this.#send({ t: "ui-request", request }, peerId);
	}
	...
	async start(relayUrl: string, webUrl = ""): Promise<void> {
		...
		socket.onClose = (_reason, willReconnect) => {
			...
			if (willReconnect) {
				...
				this.#ctx.showStatus("Collab relay connection lost, reconnecting…", { dim: true });
				return;
			}
			void this.#teardown("connection-failed");
			this.#ctx.session.emitNotice("warning", "Collab ended: relay unavailable", "collab");
		};
		socket.connect();
		...
		try {
			...
			// An explicit stop owns its teardown so it can flush the final goodbye.
			// A failed initial connection has no other teardown path.
			if (!this.#stopping && !this.#teardownDone) await this.#teardown("connection-failed");
			throw err;
		} finally {
		...
		if (!this.isReady) throw collabUnavailableError("stopped");
		...
		this.#ctx.sessionManager.onEntryAppended = this.#entryAppendHandler;
	...
	async stop(reason: string, stopReason: CollabStopReason = "user"): Promise<void> {
		...
		if (this.#stopped) return;
		this.#stopDone ??= this.#runStop(reason, stopReason);
	...
	async #runStop(reason: string, stopReason: CollabStopReason): Promise<void> {
		...
		await this.#teardown(stopReason);
	}

	#teardown(reason: CollabStopReason): Promise<void> {
		this.#teardownDone ??= this.#runTeardown(reason);
	...
	async #runTeardown(reason: CollabStopReason): Promise<void> {
		...
		const unavailableCode: CollabUnavailableCode =
			reason === "session-switch"
				...
				: reason === "connection-failed"
					...
					: "stopped";
		this.#rejectStart?.(collabUnavailableError(unavailableCode));
		this.#rejectStart = undefined;
		this.#stopped = true;
		for (const waiter of this.#readyWaiters) waiter.reject(collabUnavailableError(unavailableCode));
		...
		if (this.#ctx.collabHost === this) this.#ctx.collabHost = undefined;
		if (this.#ctx.sessionManager.onEntryAppended === this.#entryAppendHandler)
			this.#ctx.sessionManager.onEntryAppended = undefined;
		this.#entryAppendHandler = undefined;
