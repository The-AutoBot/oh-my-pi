...
export class CollabSocket {
	...
	sendRealtime(frame: CollabFrame, targetPeer = 0): boolean {
		...
		if (
			...
			(targetPeer !== 0 && !this.isServing(targetPeer))
		) {
			return false;
		}
		...
		try {
			...
			logger.debug("collab: dropping unserializable realtime frame", { error: String(err) });
			return false;
		}
		this.#realtimeSealing = true;
		void sealSerialized(this.#opts.key, serialized)
			.then(sealed => {
