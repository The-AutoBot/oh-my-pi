...
import { CollabController, type CollabEnsureResult } from "../collab/controller";
import { CollabHostStoppedError, CollabUnavailableError, type CollabHost } from "../collab/host";
...
export class InteractiveMode implements InteractiveModeContext {
	...
	set collabHost(host: CollabHost | undefined) {
		...
		if (host) this.#lastCollabHost = host;
	}
	...
	set collabGuest(guest: CollabGuestLink | undefined) {
		if (guest && guest !== this.#collabGuest)
			void this.collabController.stop("joined another collab session", "session-switch");
		this.#collabGuest = guest;
	}

	async ensureCollab(options: EnsureCollabOptions = {}): Promise<EnsureCollabResult> {
		const requestedSessionId = this.sessionManager.getSessionId();
		const activeHost = this.collabHost;
		if (activeHost?.access === "control") {
			await activeHost.waitForReady();
			if (this.collabHost !== activeHost || activeHost.sessionId !== this.sessionManager.getSessionId()) {
				throw new CollabUnavailableError("session-changed");
			}
			return Object.freeze({
				link: activeHost.link,
				webLink: activeHost.webLink,
				viewLink: activeHost.viewLink,
				webViewLink: activeHost.webViewLink,
		...
		try {
			ensured = await this.collabController.ensure({
				access: "control",
